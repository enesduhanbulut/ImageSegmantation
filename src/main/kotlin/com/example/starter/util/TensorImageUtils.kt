...
object TensorImageUtils {
     ...
     * order
     */
    suspend fun bitmapToFloat32Tensor(
        bitmap: BufferedImage, normMeanRGB: FloatArray, normStdRGB: FloatArray, memoryFormat: MemoryFormat
    ): Tensor {
        checkNormMeanArg(normMeanRGB)
        checkNormStdArg(normStdRGB)
        return bitmapToFloat32Tensor(
            bitmap, 0, 0, bitmap.width, bitmap.height, normMeanRGB, normStdRGB, memoryFormat
        )
    }
    ...
    suspend fun bitmapToFloat32Tensor(
        ...
        return bitmapToFloat32Tensor(
            bitmap, 0, 0, bitmap.width, bitmap.height, normMeanRGB, normStdRGB, MemoryFormat.CONTIGUOUS
        )
    }
    ...
    fun bitmapToFloatBuffer(
        ...
        bitmapToFloatBuffer(
            bitmap, x, y, width, height, normMeanRGB, normStdRGB, outBuffer, outBufferOffset, MemoryFormat.CONTIGUOUS
        )
    }
    ...
    suspend fun bitmapToFloat32Tensor(
        ...
        normStdRGB: FloatArray,
        memoryFormat: MemoryFormat
    ): Tensor = withContext(Dispatchers.Default) {
        checkNormMeanArg(normMeanRGB)
        checkNormStdArg(normStdRGB)
        ...
        Tensor.fromBlob(
            floatBuffer, longArrayOf(1, 3, height.toLong(), width.toLong()), memoryFormat
        )
    }

    suspend fun bitmapToFloat32Tensor(
        bitmap: BufferedImage, x: Int, y: Int, width: Int, height: Int, normMeanRGB: FloatArray, normStdRGB: FloatArray
    ): Tensor {
        return bitmapToFloat32Tensor(
